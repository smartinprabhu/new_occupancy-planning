...
interface AgentRosterMatrixProps {
  dateRange: { from: string; to: string };
  intervalAgents: number[][];
  onAgentsChange: (agents: number[][]) => void;
}

export const AgentRosterMatrix: React.FC<AgentRosterMatrixProps> = ({ 
  dateRange, 
  intervalAgents,
  onAgentsChange
}) => {
  ...
  const calculateHourlyAverage = (intervalIndex: number) => {
    if (intervalAgents.length === 0) return 0;
    const sum = intervalAgents.reduce((sum, dayAgents) => sum + (dayAgents[intervalIndex] || 0), 0);
    return Math.round(sum / intervalAgents.length);
  };

  const calculateDayTotal = (dayIndex: number) => {
    if (!intervalAgents[dayIndex]) return 0;
    return intervalAgents[dayIndex].reduce((sum, agents) => sum + agents, 0);
  };

  const calculateGrandTotal = () => {
    return intervalAgents.reduce((total, dayAgents) => 
      total + dayAgents.reduce((sum, agents) => sum + agents, 0), 0
    );
  };

  const handleAgentChange = (dayIndex: number, intervalIndex: number, value: number) => {
    const newAgents = [...intervalAgents];
    if (!newAgents[dayIndex]) newAgents[dayIndex] = new Array(48).fill(0);
    newAgents[dayIndex][intervalIndex] = value;
    onAgentsChange(newAgents);
  };

  const exportToCSV = () => {
    const headers = ['Time Interval', ...dates.map(d => d.toLocaleDateString('en-GB')), 'Hourly Avg'];
    const rows = timeIntervals.map((interval, intervalIndex) => [
      interval,
      ...dates.map((_, dayIndex) => intervalAgents[dayIndex]?.[intervalIndex] || 0),
      calculateHourlyAverage(intervalIndex)
    ]);
  ...
  return (
    <div className="bg-slate-800 rounded-lg p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          <h2 className="text-lg sm:text-xl font-semibold text-white mb-2">Agent Roster Matrix</h2>
          <p className="text-slate-400 text-sm">
            48 intervals × {dates.length} days agent roster
          </p>
        </div>
            ...
            <thead className="bg-slate-700 sticky top-0 z-20">
              <tr>
                ...
                <th className="text-center p-3 text-slate-300 font-medium bg-slate-600 sticky right-0 z-30 border-l border-slate-500">
                  <div className="text-xs text-orange-400">HOURLY AVG</div>
              ...
              {timeIntervals.map((interval, intervalIndex) => (
                <tr key={intervalIndex} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                  ...
                  {dates.map((date, dayIndex) => {
                    const agents = intervalAgents[dayIndex]?.[intervalIndex] || 0;
                    return (
                      <td key={dayIndex} className="p-2 text-center border-r border-slate-700/50">
                        <input
                          type="number"
                          value={agents}
                          onChange={(e) => handleAgentChange(dayIndex, intervalIndex, parseInt(e.target.value) || 0)}
      ...
      <div className="mt-4 text-xs text-slate-400 space-y-1">
        ...
        <div>• Date Range: {dateRange.from} to {dateRange.to}</div>
        <div>• Grand Total: {calculateGrandTotal()} agent intervals</div>
      </div>
    </div>
  );
};
